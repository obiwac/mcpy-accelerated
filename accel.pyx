	...
	for local_x in range(SUBCHUNK_WIDTH):
		for local_y in range(SUBCHUNK_HEIGHT):
			for local_z in range(SUBCHUNK_LENGTH):
				...
				block_number = self.parent.get_block(parent_lx, parent_ly, parent_lz)
				...
				if block_number:
					...
					if block_type.is_cube:
						if can_render_face(chunks, block_types, block_number, block_type, (x + 1, y, z)): add_face(0)
						if can_render_face(chunks, block_types, block_number, block_type, (x - 1, y, z)): add_face(1)
						if can_render_face(chunks, block_types, block_number, block_type, (x, y + 1, z)): add_face(2)
						if can_render_face(chunks, block_types, block_number, block_type, (x, y - 1, z)): add_face(3)
						if can_render_face(chunks, block_types, block_number, block_type, (x, y, z + 1)): add_face(4)
						if can_render_face(chunks, block_types, block_number, block_type, (x, y, z - 1)): add_face(5)
